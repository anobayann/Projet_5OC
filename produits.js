//JSON.parse -> JSON string to javascript obj
...
.then(res =>  {
  ...
  if (res.ok){
       res.json().then(teddies =>{
            ...
            if (document.URL.indexOf("personaliser.html?_norbert") >= 0){ 
...
function addToLocalstorage(){
    ...
    button.addEventListener("click", function() {
          ...
          const addTeddy = JSON.parse(localStorage['cart'] || "[]")
                // on cree l'objet product qui represente le teddy selectionne et ses characteristiques
          ...
          let product = {
                ...
                // creation d'une variable pour verifier si le produit est deja dans le panier avec son _id
          let exisisting = addTeddy.find(item => item._id == product._id)
                // si le produit existe on additionne juste sa quantite sinon on ajoute le nouveau produit
          if (exisisting){
                ...
                // on ajoute la valeur (l objet(teddy)) a la cle(panier cart) au localStorage
